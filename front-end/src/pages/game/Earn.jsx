...
function Earn() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      const response = await axios.get(`https://tap-genie-app-production.up.railway.app/api/earn/getscore`,{
  ...
  const syncWithServer = async (energy, points, restore_time) => {
    ...
    if (user) {
      ...
      if( points>0){
        await axios.post(
          `https://tap-genie-app-production.up.railway.app/api/earn/upscore`,
          {
            score: points,
  ...
  <div className="flex">
    <div className="relative flex">
      <motion.img
        ...
        src={CoinImg}
        alt="Coin"
        className="img-fluid animate__animated animate__bounce small:w-52 small:h-52 h-70 w-70 z-10 rounded-full select-none"
